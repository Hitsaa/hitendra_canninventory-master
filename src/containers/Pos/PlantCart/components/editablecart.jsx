...
export default class Editablecart extends PureComponent {
  ...
  deleteProduct = item => () => {
    const apiUrl = `${process.env.API_ENDPOINT}/pos/plants/cart/${item.cart_id}/plant/${item.plant_id}/delete`;
    const tokenStr = localStorage.getItem('cannave_token');
    const token = JSON.parse(tokenStr);
  ...
  checkoutProducts() {
    ...
    const apiUrl = `${process.env.API_ENDPOINT}/pos/plants/checkout/${cartresponse.id}`;
    const tokenStr = localStorage.getItem('cannave_token');
    const token = JSON.parse(tokenStr);
    const bodyFormData = new FormData();
              ...
              <Col>
                <Tooltip title="back">
                  <IconButton onClick={() => edit()}>
                    <Col><BackIcon /> Add More Plants</Col>
                  </IconButton>
                </Tooltip>
              </Col>
            </Row>
            <div className="editableItemDiv">  Cart is empty , Please add plants</div>
            </Card>
        </Container>
      )
    }else{
    const plants = cartresponse.products;
    const { PaymentMode } = this.state;
            ...
            <div className="editableItemDiv">
              ...
              <Row className="total-Box-Row">
                <Col lg={6} className="col-VnH-center">
                  <div className="plants-margin-top2p">
