...
const columns = ['POS', 'First Name', 'Last Name', 'Email Address', 'Address', 'Phone', 'Zip Code', 'City', 'State'];
      ...
      if (!response.data.error) {
        ...
        for (let i = 0; i < response.data.users.length; i += 1) {
          rows.push({
            ...
            first_name: response.data.users[i].client.firstName,
            last_name: response.data.users[i].client.lastName,
            ...
            pos: <Tooltip title="Add"><IconButton component={Link} to={{ pathname: '/pos/AddtoCart', state: { data: response.data.users[i], manage: false, selectedCartID: '056fgtb' } }}><ShoppingCart /></IconButton></Tooltip>,
          });
        }
      } else {
        this.setState({
      ...
      type = (
        <Col md={12} lg={12}>
          <DataPaginationTable2
            ...
            data={rows.map((item) => { return [item.pos, item.first_name,item.last_name, item.email, item.address, item.phone, item.zip, item.city, item.state]; })}
            columns={columns}
            options={options}
          />
        </Col>
