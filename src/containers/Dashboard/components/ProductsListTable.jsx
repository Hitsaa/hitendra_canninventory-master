...
const columns = ['Id', 'Strain Name', 'Weight IN', 'Weight', 'Price By Weight', 'Category', 'Batch Serial NO#', 'Edit', 'Delete'];
...
export default class ProductsListTable extends PureComponent {
  ...
  createRows = async () => {
    ...
    const url = `${process.env.API_ENDPOINT}/products/summary`;
    await axios.get(url, {
      headers: {
      ...
      type = (
        <Col md={12} lg={12}>
          <DataPaginationTable2
            ...
            data={rows.map((item) => { return [item.id, item.strain_name, item.weight_type, item.weight, item.price, item.category, item.batch_no, item.edit, item.delete]; })}
            columns={columns}
            options={options}
          />
        </Col>
