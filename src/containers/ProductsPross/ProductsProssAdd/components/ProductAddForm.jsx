...
import { Button, ButtonToolbar, Col, Input } from "reactstrap";
...
class ProductAddForm extends PureComponent {
  constructor() {
    ...
    this.state.serial_no = this.generateSerialSimple('serial_no');
    this.state.batch_lot = this.generateSerialSimple('batch_lot');
  ...
  async componentDidMount() {
    ...
    if (user !== null && user !== "" && user !== undefined) {
      if (JSON.parse(user)) {
        await this.createProcessorProduct();
  ...
  createProcessorProduct = async () => {
    // let inventory = {};
    const tokenStr = localStorage.getItem("cannave_token");
    const token = JSON.parse(tokenStr);
    const url = `${process.env.API_ENDPOINT_LOC}/processor-product-create`;
    await axios
      .get(url, {
        headers: {
  ...
  inputChangedHandler(event, state) {
    if (state === 'batchID') {
      this.setState( {batch_lot: event.target.value} );
  ...
  async handleSubmitGrowProc(e) {
    ...
    let weightType = "";
    let quantity = "";
    console.log(this.refs.ProcessorAddForm);
    if (!this.state.shouldShow) {
      batchNo = this.state.batch_lot;
      strainName = this.refs.ProcessorAddForm.product_name.value;
      serial = this.state.serial_no;
      plantStage =
        this.refs.ProcessorAddForm.plant_stage &&
        this.refs.ProcessorAddForm.plant_stage.value
          ? this.refs.ProcessorAddForm.plant_stage.value
          : "";
    } else {
      strainName = this.refs.ProcessorAddForm.product_name.value;
      serial = this.state.serial_no;
      componentTracking = this.refs.ProcessorAddForm.room_type.value;
      plantStage = this.refs.ProcessorAddForm.plant_stage.value;
      batchNo =
        this.refs.ProcessorAddForm.batch_lot &&
        this.refs.ProcessorAddForm.batch_lot.value
          ? this.refs.ProcessorAddForm.batch_lot.value
          : this.state.batch_lot;
      growMethod = this.refs.ProcessorAddForm.grow_method.value;
      // console.log(growMethod);
      shouldCreatePlant = true;
    }
    businessName = this.refs.ProcessorAddForm.business_name.value;
    if (this.state.shouldCreateClient) {
      ...
      businessLicense = this.refs.ProcessorAddForm.business_license_number
        .value;
      role = this.inputRole.value;
    }
    // if (this.state.isPlant) {
    quantity = this.refs.ProcessorAddForm.quantity.value;
    // } else {
    weight = 0;
    if(typeof this.refs.ProcessorAddForm.weight_per_sellable_product !== "undefined"){
      weight = this.refs.ProcessorAddForm.weight_per_sellable_product.value;
    }
    weightType = 0;
    if(typeof this.refs.ProcessorAddForm.weight_type !== "undefined"){
      weightType = this.refs.ProcessorAddForm.weight_type.value;
    }
    // }

    const categoryID = this.refs.ProcessorAddForm.category_id.value;
    const pricePerWeight = this.refs.ProcessorAddForm.price_per_unit_weight
      .value;
    const roomType = this.refs.ProcessorAddForm.room_type.value;
    const processorPlantStage = this.refs.ProcessorAddForm.plant_stage.value;

    const subInventoryIds = this.refs.ProcessorAddForm.inventory_id;
    const inventoryWeightTypes = this.refs.ProcessorAddForm
      .inventory_weight_type;
    const inventoryQuantities = this.refs.ProcessorAddForm.inventory_quantity;
    ...
    bodyFormData.append("plant_stage", processorPlantStage.value);
    ...
    // businessLicense = this.refs.ProcessorAddForm.business_license_number.value;
    bodyFormData.append("business_license_number", this.state.businessLicense);
    ...
    if (typeof inventoryQuantities.length !== "undefined") {
      for (let i = 0; i < subInventoryIds.length; i += 1) {
        ...
        bodyFormData.append(
          `processor_product_inventory[${i}][inventory_id]`,
          subInventoryIds[i].value
        );
        bodyFormData.append(
          `processor_product_inventory[${i}][inventory_weight_type]`,
          inventoryWeightTypes[i].value
        );
        bodyFormData.append(
          `processor_product_inventory[${i}][inventory_quantity]`,
          inventoryQuantities[i].value
        );
      }
    } else {
      bodyFormData.append(
        "processor_product_inventory[0][inventory_id]",
        subInventoryIds.value
      );
      bodyFormData.append(
        "processor_product_inventory[0][inventory_weight_type]",
        inventoryWeightTypes.value
      );
      bodyFormData.append(
        "processor_product_inventory[0][inventory_quantity]",
        inventoryQuantities.value
      );
    }

    const urls = `${process.env.API_ENDPOINT_LOC}/processor-product-store`;
  ...
  generateSerialSimple(param) {
    ...
    if (param === 'batch_lot') {
      ...
      this.setState({ batch_lot: randomSerial });
    } else if (param === 'serial_no') {
      randomSerial = `SN${randomSerial}`;
      this.setState({ serial_no: randomSerial });
    } 
  ...
  async handleCategorySubmit(e) {
    ...
    if (
      this.state.categories[categoryID].toLowerCase() === "ancillary" ||
      this.state.categories[categoryID].toLowerCase() === "other"
    ) {
      this.setState({ shouldSearch: false });
      // this.setState({ isOtherProduct: true });
      this.setState({ isOtherProduct: false });
  ...
  render() {
    ...
    const { selectedManufacturer,businessLicense} = this.state;
      ...
      if (showCheckCategory) {
            ...
            <div className="form__form-group">
              ...
              <div className="form__form-group-field">
                <Input
                  ...
                  onChange={(event)=>this.inputChangedHandler(event, 'CategoryID')}
                  name="category_id"
        ...
        if (isOtherProduct) {
          type = (
            <form
              className="form form form--horizontal"
              onSubmit={this.handleSubmitOtherProduct}
            >
              <div className="form__form-group">
          ...
          for (let i = 0; i < this.state.inventoryCount; i += 1) {
            inventoryDivs.push(
              <RelatedInventory
                key={i}
                weightTypes={this.state.weightTypes}
                allInventories={this.state.inventories}
                index={i}
              />
            );
          }
            ...
            >
                  ...
                  <div className="form__form-group-field">
                    {/* <Input
                      ...
                      <Input
                        ...
                        name="business_name"
                        required
                        // value={selectedManufacturer.businessName}
                        ref={node => (this.inputBusinessName = node)}
              ...
              <Col md={6} lg={6}>
                {/* <div className="form__form-group">
                  ...
                  <div className="form__form-group-field">
                    <span>{(this.state.businessLicense) ? this.state.businessLicense :`N/A` }</span> */}
                    {/* <Input
                        type="text"
                      ...
                      {/* <Input
                        type="select"
                        value={this.state.selectedManufacturer.businessLicense}
                        ref={node => (this.inputBusinessLicense = node)}
                        name="business_license_number"
                        className="form-control-sm react-select"
                        required
                  ...
                  <div className="form__form-group form__form-group-id">
                    ...
                    <div className="form__form-group-field">
                      ...
                      >
                        ...
                        <option key="HYDROPONIC" value="HYDROPONIC">
                          HYDROPONIC
                        </option>
                      </Input>
                    </div>
                  ...
                  <div className="form__form-group">
                    ...
                    <div className="form__form-group-field">
                      ...
                      >
                        ...
                        <option key="INDOOR" value="1">
                          INDOOR
                        </option>
                        <option key="OUTDOOR" value="2">
                          OUTDOOR
                        </option>
                        <option key="GREENHOUSE" value="3">
                          GREENHOUSE
                        </option>
                      </Input>
                    </div>
                  ...
                  <div className="form__form-group">
                    ...
                    <div className="form__form-group-field">
                      <Input
                        ...
                        required
                      >
                        {this.state.weightTypes.map((Value, Index) => (
                          <option value={Index} key={Value}>
                            {Value}
                          </option>
                        ))}
              ...
              <Col md={6} lg={6}>
                  ...
                  <div className="form__form-group">
                    ...
                    <div className="form__form-group-field">
                      ...
                      >
                        ...
                        <option key="SEEDLING" value="SEEDLING">
                          SEEDLING
                        </option>
                        <option key="PRE-VEG" value="PRE-VEG">
                          PRE-VEG
                        </option>
                        ...
                        <option key="DRYING" value="DRYING">
                          DRYING
                        </option>
                        <option key="DRIED" value="DRIED">
                          DRIED
                        </option>
                        <option key="SOLD" value="SOLD">
                          SOLD
                        </option>
                        <option key="DESTROYED" value="DESTROYED">
                          DESTROYED
                        </option>
                      </Input>
                    </div>
                ...
                {/* <div className="form__form-group">
                  ...
                  <div className="form__form-group-field">
                    <Field
                      name="weight_per_sellable_product"
                      component="input"
                      type="text"
                    />
                  </div>
                ...
                <div className="form__form-group">
                  ...
                  <div className="form__form-group-field">
                    <Input
                      ...
                      required
                      ref={node => (this.inputPricePerUnitWeight = node)}
                ...
                <div className="form__form-group">
                  <div className="form__form-group-field">
                    <Button
                      ...
                      type="button"
                      onClick={() =>
                        this.addInventoryFields(this.state.inventoryCount)
                      }
                    >
                      Add Inventory
                    </Button>
                    <Button
                      ...
                      type="button"
                      onClick={() =>
                        this.removeInventoryFields(this.state.inventoryCount)
                      }
                    >
                      Remove Inventory
                    </Button>
...
function RelatedInventory(props) {
  ...
  return (
    <div className="ingredient_field-set" key={props.index}>
      <h4 className="text-center">Inventory - {props.index + 1}</h4>
      <br />
      <div className="form__form-group">
        ...
        <div className="form__form-group-field">
          ...
          >
            ...
            {props.allInventories.map(Value => (
      ...
      <div className="form__form-group">
        <span className="form__form-group-label">Inventory Weight Type</span>
        <div className="form__form-group-field">
          <Input
            type="select"
            name="inventory_weight_type"
            id={InventoryWeightType}
            className="form-control-sm react-select"
          >
            <option value="00">Select Weight Type</option>
            {props.weightTypes.map((Value, Index) => (
              <option value={Index} key={Value}>
                {Value}
              </option>
            ))}
